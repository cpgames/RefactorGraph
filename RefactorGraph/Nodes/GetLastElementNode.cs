...
{
    [Node]
    [RefactorNode(group = RefactorNodeGroup.Logic, nodeType = RefactorNodeType.GetLastElement)]
    [NodeFlowPort(INPUT_PORT_NAME, "", true)]
    [NodeFlowPort(OUTPUT_PORT_NAME, "", false)]
    public class GetLastElementNode : RefactorNodeBase
    {
        #region Fields
        ...
        public override void OnExecute(Connector prevConnector)
        {
            ...
            {
                SetPortValue(CHUNK_PORT_NAME, collection[collection.Count - 1]);
                _success = true;
            }
        }
